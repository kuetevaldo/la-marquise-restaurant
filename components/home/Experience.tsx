    ...
    >
      <div className="mx-auto w-full max-w-7xl px-6 sm:px-8 md:px-10 lg:px-12 xl:px-16">
        ...
        <div className="mx-auto grid w-full max-w-6xl gap-10 lg:grid-cols-2 lg:items-stretch lg:gap-10 xl:gap-12">
          ...
          {/* Main image */}
          <Reveal y={30}>
            <div className="relative min-h-105 w-full overflow-hidden bg-(--surface-elevated) sm:min-h-130 lg:min-h-160">
              <Image
              ...
              <div className="absolute bottom-6 left-6 right-6 sm:bottom-8 sm:left-8 sm:right-8">
                <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-(--brand-light)">
                  Bonapriso · Douala
                </p>

                <h3 className="font-display mt-3 max-w-md text-3xl leading-tight text-(--foreground) sm:text-4xl">
                  Un cadre pensé pour
            ...
            <Reveal delay={0.08} y={18}>
              <div className="mt-8 border-t border-(--border)">
                {experiences.map((item) => (
                  <div
                    key={item}
                    className="flex items-center justify-between gap-6 border-b border-(--border) py-4 sm:py-5"
                  >
                    ...
                    <span
                      aria-hidden="true"
                      className="h-px w-8 shrink-0 bg-(--brand) opacity-60"
                    />
                  </div>
                ))}
              </div>
